...
export default function ImageCompressTool() {
  ...
  return (
    <ToolPageShell title="이미지 용량 줄이기" description="품질을 낮춰 JPEG로 다시 저장합니다. 파일은 브라우저에서만 처리됩니다.">
      <FileDropzone accept="image/*" files={files} onChange={next => { setFiles(next); setResult(null); }} hint="JPG, PNG, WebP 등" />

      <div className="tool-controls">
        <label className="field">
          <span className="field__label">품질 {Math.round(quality * 100)}%</span>
          <input className="field__range" type="range" min={0.1} max={0.95} step={0.05} value={quality} onChange={e => setQuality(Number(e.target.value))} />
        </label>
      </div>
      ...
      {file && result ? (
        <div className="preview-box">
          ...
          <div className="preview-meta">
            ...
            <span>{Math.round((result.blob.size / file.size) * 100)}%</span>
          </div>
        </div>
      ) : null}
    </ToolPageShell>
