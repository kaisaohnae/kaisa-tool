...
export default function PdfCompressTool() {
  ...
  const run = async () => {
    ...
    setStatus('페이지 렌더링 중…');
    try {
      const blob = await compressPdf(file, quality, 1.25);
      setResult({blob, name: replaceExtension(file.name, 'pdf').replace(/\.pdf$/i, '-compressed.pdf')});
      setStatus('');
    } catch (e) {
    ...
    <ToolPageShell title="PDF 압축" description="각 페이지를 이미지로 다시 담아 용량을 줄입니다. 텍스트 선택·벡터는 유지되지 않을 수 있습니다.">
      ...
      <div className="tool-controls">
        <label className="field">
          ...
          <input className="field__range" type="range" min={0.3} max={0.95} step={0.05} value={quality} onChange={e => setQuality(Number(e.target.value))} />
        </label>
      </div>
